...
def process_dataset_item(
	...
	try:
		...
		patched_triton_code = write_modified_program(python_code, entry_point, triton_code)
...
def _process_and_filter_item(args):
	...
	"""
	item_id, item, process_func, passed_queue, failed_queue, filter_key, error_queue = args
	try:
		num_gpus = torch.cuda.device_count()
...
def process_and_filter_parallel(
	...
	with Timer(f'Parallel processing and filtering - {desc}'):
		...
		with mp.Pool(num_workers, maxtasksperchild=1) as pool:
			results = [pool.apply_async(_process_and_filter_item, (arg,)) for arg in process_args]
...
class TritonDatasetGenerator:
	...
	def __init__(
		...
		self.cleaned_pytorch_modules_dir = os.path.join(run_dir, 'cleaned_pytorch_modules')
	...
	def generate_dataset(self) -> Optional[Tuple[List[Dict], List[Dict], List[Dict]]]:
		...
		with Timer('Total dataset generation'):
			...
			if self.limit:
				print(f'Limiting to {self.limit} files of total {len(file_tasks)} files')
				file_tasks = file_tasks[: self.limit]

			# Save file tasks for reference
			file_tasks_path = os.path.join(self.intermediate_datasets_dir, 'file_tasks.json')
			...
			print(f'Found {len(tracking_dicts)} triton modules that were able to be cleaned')
			...
			with Timer('Filtering outcode with more than one return statement'):
				...
				filtered_dataset_returns, bad_return_data, error_dict = process_and_filter_parallel(
		...
		with Timer('Transforming code to kernel bench format'):
			for dataset_entry in tqdm(filtered_dataset, desc='Transforming code'):
				dataset_entry['python_code'] = transform_get_functions(dataset_entry['python_code'])

		# Filter for correctness
		with Timer('Evaluating correctness'):
			dataset, modules_which_failed_correctness, error_dict = process_and_filter_parallel(
				filtered_dataset,
				check_dataset_entry,
				self.num_concurrent_gpu_jobs,
		...
		print(f'After filtering for correctness produced dataset of size: {len(dataset)}')
		print(f'Failed to evaluate {len(modules_which_failed_correctness)} modules for correctness')
		...
		dataset_scrape = copy.deepcopy([entry for entry in dataset if entry['synthetic'] is False])
...
def main():
	...
	parser = argparse.ArgumentParser(description='Generate a dataset of Python-Triton code pairs')
	parser.add_argument(
		'--run-dir',
		default='./runs/run1',
	...
	parser.add_argument('--jobs', type=int, default=8, help='Number of parallel workers / jobs')
	...
	with Timer('Overall execution'):
		...
		with Timer('Saving dataset to parquet'):
			if len(dataset_scrape) > 0:
				...
				output_file_scrape = os.path.join(datasets_dir, 'scrape_dataset.parquet')
				df_scrape.to_parquet(output_file_scrape, index=True)

			if len(dataset_synthetic) > 0:
				df_synthetic = pd.DataFrame(dataset_synthetic)
				output_file_synthetic = os.path.join(datasets_dir, 'synthetic_dataset.parquet')
