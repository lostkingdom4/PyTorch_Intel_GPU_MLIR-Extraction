...
def evaluate_ref_and_kernel_correctness(
	...
	"""
	set_deterministic()
	model, get_inputs, get_init_inputs = import_Model_and_args_from_code(ref_arch_src, entry_point)
	...
	triton_model_class, temp_file = import_ModelNew_from_code(kernel_src, f'{entry_point}New')
	...
	for _ in range(num_trials):
		...
		for ref_out, triton_out in zip(ref_output, triton_output):
			if not isinstance(ref_out, torch.Tensor):
				# close file
				print('an output of the forward pass of the reference model is not a tensor')
				temp_file.close()
				return False
			is_close = (
			...
			if not isinstance(triton_out, torch.Tensor):
				# close file
				print('an output of the forward pass of the triton model is not a tensor')
				temp_file.close()
				return False
...
def main():
	...
	parser.add_argument('--uuid', type=int, required=True, help='UUID of kernel to evaluate')
	parser.add_argument(
		'--pytorch_save_path',
		type=str,
		default=None,
	...
	check = evaluate_ref_and_kernel_correctness(pytorch_code, triton_code, entry_point, 100)
	if check:
